...
class QdrantService:
    """
    Production-grade Qdrant service for Legal RAG systems.
    ...
    def upsert_chunks(
        ...
        if len(chunks) != len(metadata_list):
            raise ValueError(
                "Chunks/metadata count mismatch."
            )

        total_points = len(chunks)

        logger.info(
            f"Preparing {total_points} vectors for Qdrant upsert."
        )
    ...
    def delete_by_case(self, case_id: str):
        ...
        try:
            ...
            logger.info(
                f"Deleted vectors for case_id={case_id}"
            )

        except Exception:
            logger.exception(
                f"Failed deleting vectors for case={case_id}"
            )
            raise